...
async function dev(){
    const subFileLs = await fs.readdir('/mnt/nfs/db_book', {encoding: 'utf-8', withFileTypes: true});
    subFileLs.forEach(subFile => {
        console.info(subFile.name,subFile.isDirectory(),subFile.isFile());
    });
}
dev();
